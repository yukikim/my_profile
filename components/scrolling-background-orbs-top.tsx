...
export function ScrollingBackgroundOrbs() {
  ...
  useEffect(() => {
    ...
    const updateOrbPositions = () => {
      ...
      if (emeraldOrbRef.current) {
        emeraldOrbRef.current.style.transform = `translate3d(${scrollY * 0.02}px, ${scrollY * 0.01}px, ${scrollY * 0.4}px)`;
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none fixed perspective-midrange -z-9 left-[-4%] top-[-3%]">
      <div
        ref={amberOrbRef}
        className="fixed top-[-20%] lg:top-[-20%] left-[-30%] lg:left-[0%] -z-10 h-100 w-100 rounded-full bg-amber-400/35 blur-none will-change-transform"
      />
      <div
        ref={emeraldOrbRef}
        className="fixed top-[10%] left-[10%] -z-11 h-160 w-160 rounded-full bg-emerald-500/30 blur-none will-change-transform"
      />
      <div
        ref={tealOrbRef}
        className="fixed top-[10%] left-[-30%] lg:left-[0%] -z-12 h-600 w-600 rounded-full bg-teal-400/30 blur-none will-change-transform"
      />
    </div>
  );
}
