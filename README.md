...
| Engineering Notes | `/engineering-notes`                  | 公開可能な開発日誌と設計判断を一覧表示する           |
| 開発日誌詳細      | `/engineering-notes/logs/[slug]`      | 実装内容、問題、原因、解決方法、学びを表示する       |
| ADR詳細           | `/engineering-notes/decisions/[slug]` | 設計背景、候補、判断理由、影響を表示する             |
...
| FE-009 | Engineering Notes一覧 | `/engineering-notes`                  | 公開済み開発日誌とADR                           |
| FE-010 | 開発日誌詳細          | `/engineering-notes/logs/[slug]`      | 実装、問題、原因、解決、学び、関連情報          |
| FE-011 | ADR詳細               | `/engineering-notes/decisions/[slug]` | 背景、候補、判断、理由、影響、関連情報          |
...
## 15. Engineering Notes・設計判断MCP

### 15.1 概要

開発過程を`DevelopmentLogs`、設計判断を`ArchitectureDecisions`としてPayload CMSへ保存する。両Collectionは既存の`Works`と関連付けられる。

公開範囲は次の3段階で分離する。

| 状態                | 公開サイト | 信頼済みローカルMCP | Payload管理画面 |
| ------------------- | ---------- | ------------------- | --------------- |
| public + published  | 表示する   | 取得できる          | 取得できる      |
| private + published | 表示しない | 取得できる          | 取得できる      |
| draft               | 表示しない | 取得しない          | 取得できる      |
...
公開サイトと管理画面は次のコマンドで起動する。

```bash
npm run dev
```

MCPサーバーだけをstdioで起動する場合は次を実行する。

```bash
npm run mcp
```

Codexはプロジェクトの`.codex/config.toml`から`engineering_notes`サーバーを読み込む。設定変更後はCodexを再起動し、プロジェクトrootで次を確認する。
